...
- `VOXINPUT_LOCALVQE_MODEL_VERSION`: Which bundled LocalVQE model to use: `v1.2` (default) or `v1.3`. The full `version-size` form (`v1.2-1.3M`, `v1.3-4.8M`) is also accepted. The CMake build bundles both models under `share/voxinput`; with a plain `go build` the chosen model is downloaded from HuggingFace on first use, verified against a known checksum, and cached under the user cache directory (e.g. `~/.cache/voxinput`). Ignored when `VOXINPUT_LOCALVQE_MODEL` is set.
